...
export default function ProductsByCategory() {
  ...
  const checkScroll = () => {
    const scrollContainer = document.querySelector('.categories-scroll') as HTMLElement;
    if (scrollContainer) {
  ...
  const allProducts = selectedCategory ? getProductsByCategory(selectedCategory.url) : [];
  const productCount = selectedCategory ? getProductsCountByCategory(selectedCategory.url) : 0;
      ...
      <div className="relative mb-8">
        ...
        {showLeftScroll && (
          <button
            ...
            className="absolute left-0 top-1/2 transform -translate-y-1/2 z-30 bg-white/95 backdrop-blur-sm border border-gray-300 rounded-full w-10 h-10 flex items-center justify-center shadow-lg hover:bg-white hover:shadow-xl transition-all duration-200 hover:scale-110"
        ...
        {showRightScroll && (
          <button
            ...
            className="absolute right-0 top-1/2 transform -translate-y-1/2 z-30 bg-white/95 backdrop-blur-sm border border-gray-300 rounded-full w-10 h-10 flex items-center justify-center shadow-lg hover:bg-white hover:shadow-xl transition-all duration-200 hover:scale-110"
        ...
        <div className="categories-scroll flex overflow-x-auto scrollbar-hide gap-2 px-10 py-2">
          {categories.map((category) => {
            ...
            const categoryProductCount = getProductsCountByCategory(category.url);
            ...
            return (
              <motion.button
                ...
                className={`
                  flex items-center gap-3 px-4 py-3 rounded-xl border-2 transition-all duration-300 flex-shrink-0 whitespace-nowrap
                  ${isSelected 
              ...
              >
                ...
                <div className="flex flex-col items-start">
                  ...
                  <span className="text-xs opacity-70">{categoryProductCount} productos</span>
                </div>
              </motion.button>
            );
          })}
                ...
                <AnimatePresence>
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {/* Imagen */}
                          <div className="p-0 relative h-48">
                            {product.imagenes_urls && product.imagenes_urls[0] ? (
                              <img
                                src={product.imagenes_urls[0]}
                                alt={product.nombre}
                                className="w-full h-full object-cover transition-transform duration-500 hover:scale-110"
                              ...
                              <div className="w-full h-full flex items-center justify-center bg-gray-50">
                                <Icon className={`text-4xl opacity-20 ${selectedCategory.color}`} />
                              </div>
                            )}
                          ...
                          <CardBody className="p-5">
                            ...
                            <div className="space-y-2 mb-4">
                              ...
                              {product.material?.exterior && (
                                <div className="flex items-center gap-2 text-sm text-gray-500">
                                  <div className="w-1.5 h-1.5 bg-green-500 rounded-full"></div>
                                  <span>Material: {product.material.exterior}</span>
                                </div>
                              )}
                            </div>
                            ...
                            {product.colores_disponibles && product.colores_disponibles.length > 0 && (
                              <div className="mb-4">
                                <div className="flex items-center gap-2 mb-2">
                                  ...
                                  <div className="flex gap-1">
                                    {product.colores_disponibles.slice(0, 4).map((color, idx) => (
                                      <div
                                        key={idx}
                                        className="w-4 h-4 rounded-full border border-gray-200 shadow-sm"
                  ...
                  {allProducts.length > 6 && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      whileHover={{ scale: 1.05 }}
                      onClick={() => window.open(`/categories/${selectedCategory.url}`, "_self")}
                      className="cursor-pointer flex flex-col items-center justify-center border-2 border-dashed border-blue-400 rounded-2xl p-8 text-center text-blue-600 hover:bg-blue-50 transition-all"
                    >
